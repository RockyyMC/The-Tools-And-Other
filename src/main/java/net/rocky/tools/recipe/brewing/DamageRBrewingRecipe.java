...
@EventBusSubscriber
public class DamageRBrewingRecipe implements IBrewingRecipe {
	@SubscribeEvent
	public static void init(RegisterBrewingRecipesEvent event) {
		event.getBuilder().addRecipe(new DamageRBrewingRecipe());
	}
	...
	public boolean isIngredient(ItemStack ingredient) {
		return Ingredient.of(TheToolsAndOtherModItems.COPPERNUGGET.get()).test(ingredient);
	}

	@Override
	public ItemStack getOutput(ItemStack input, ItemStack ingredient) {
		if (isInput(input) && isIngredient(ingredient)) {
			return PotionContents.createItemStack(input.getItem(), TheToolsAndOtherModPotions.DAMAGE_P);
		}
