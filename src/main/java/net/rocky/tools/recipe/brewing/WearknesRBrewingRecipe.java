...
@EventBusSubscriber
public class WearknesRBrewingRecipe implements IBrewingRecipe {
	@SubscribeEvent
	public static void init(RegisterBrewingRecipesEvent event) {
		event.getBuilder().addRecipe(new WearknesRBrewingRecipe());
	}
	...
	public boolean isIngredient(ItemStack ingredient) {
		return Ingredient.of(Blocks.SEA_PICKLE.asItem()).test(ingredient);
	}

	@Override
	public ItemStack getOutput(ItemStack input, ItemStack ingredient) {
		if (isInput(input) && isIngredient(ingredient)) {
			return PotionContents.createItemStack(input.getItem(), TheToolsAndOtherModPotions.WEARKNES);
		}
